...
class AVLTree:
    def __init__(self, root: Optional[TNode], linked_list=None):
        self.root = root
        if linked_list:
            for node in linked_list:
                self.insert(TNode(key=node.pair.key, value=node.pair.value))

    def __iter__(self):
        def iter_helper(node=self.root):
            if node:
                yield node
    ...
    def find(self, value):
        def find_helper(value, root=self.root):
            if not root:
                return None
            if root.pair.key == value:
                return root
            if value > root.pair.key:
                return find_helper(value, root.right)
            return find_helper(value, root.left)
        return find_helper(value)
    ...
    def rank(self, root, node, rsum=0):
        def rank_helper(root, keynode):
            total_sum = 0
            if not root:
                return 0
            if root != keynode:
                total_sum += 1
            if root.left and root.left == keynode:
                total_sum += rank_helper(root.left.left, keynode)
            else:
                total_sum += rank_helper(root.left, keynode)
            if root.right and root.right == keynode:
                total_sum += rank_helper(root.right.right, keynode)
            else:
                total_sum += rank_helper(root.right, keynode)
        ...
        if node < root or (node == root and node.value > root.value):
            ...
            if root.right:
                ...
                if root.right.left and root.right.left == node:
                    noteqls -= 1
            newrsum = rsum + noteqls
            if node != root:
                newrsum += 1
            return self.rank(root.left, node, newrsum)
...
def print_tree(root):
    ...
    for i, l in enumerate(levels):
        ...
        seg = width // (pow(2, i + 1))
        for n in l:
            valstr = str(n[0].pair.key) + f"({n[0].height},{n[0].size})"
            if n[3] == "r":
                linestr += (
