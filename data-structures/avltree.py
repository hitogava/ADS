...
class AVLTree:
    ...
    def rank(self, root, node, rsum=0):
        ...
        if not root:
            return 0
        if root is node:
            # noteqls = 0 if not root.right else rank_helper(root.right, node)
            noteqls = 0
            if root.right:
                ...
                if root.right.left and root.right.left == node:
                    noteqls -= 1
            # noteqls = 0 if not root.right else root.right.size
            return rsum + noteqls
...
# tree = AVLTree(None)
# vals = [3, 3, 2, 3, 5, 3, 2, 1, 2, 8]
# # vals = [1, 1, 1, 1, 2, 2]
# i = 0
# for v in vals:
#     tree.insert(Node(key=v, value=str(i)))
#     i += 1
# print_tree(tree.root)
# for v in vals:
#     t = tree.find(tree.root, v)
#     print(f"Rank of {v}: {tree.rank(tree.root, t)}")
#
#
